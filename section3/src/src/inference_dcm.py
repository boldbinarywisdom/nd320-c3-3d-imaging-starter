...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                         ...
                         f"Study Desc: {header.StudyDescription}\n" \
    ...
    nd_tmp = np.flip((orig_vol[0, :, :]/np.max(orig_vol[0, :, :]))*0xff).T.astype(np.uint8)

    # This is how you create a PIL image from numpy array
    #__solution 64, 64 is a guess to have an image small enough but usable
    # it also must accommodate the bounding box below

    pil_tmp = Image.fromarray(nd_tmp, mode="L").convert("RGBA").resize((640, 640))
    
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pil_tmp, box=(400, 400))
...
def get_series_for_inference(path):
    ...
    print("get_series_for_inference")
    ...
    # <YOUR CODE HERE>
    # __ solution
    path_to_series = [dir for dir, subdirs, files in os.walk(path) if 'HCropVolume' in dir]
    selected_path = np.random.choice(path_to_series)
    series_for_inference = [pydicom.dcmread(os.path.join(path, f)) for f in os.listdir(selected_path)]
    ...
    print("from get_series_for_inference: done")
